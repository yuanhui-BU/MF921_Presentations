...
\begin{frame}{Part I}
    ...
    \end{center}
    \vspace{3em}
    \par Focus on section 2 and 4. The model setup, the pricing and Laplace transforms formula of Barrier 
    and Lookback option and the numerical results based on Gaver-Stehfest (GS) method.
 \end{frame}
...
\begin{frame}{Part II}
    ...
    \end{center}
    \vspace{3em}
    \par  An alternative two-dimensional Laplace transforms of Barrier and Lookback option prices
    and the numerical results based on two-sided Euler method.
...
\begin{frame}{Intuition of the Pricing Formula}
    ...
    \vspace{1em}
    \item  \pause With jumps, however, analytical pricing becomes difficult 
...
\begin{frame}{Intuition of the Pricing Formula Con}
    ...
    \vspace{1em}
    \par  \pause This overshoot creates complications to compute the distribution of the first passage
times analytically: 
    \begin{itemize}
    ...
    \end{itemize}
    \vspace{1em}
    \par  \pause \textbf{Note}: Double exponential distribution assumption has a memoryless property, this property 
...
\begin{frame}{Lookback Options Con.}
    ...
    \par This is the Bromwich inversion integral, intractable in closed form.
     ...
     \par  \pause The most widely used numerical methods for Laplace transform inversion is
...
\begin{frame}{Lookback Options Con.}
    ...
    \vspace{1em}
    \par  \pause The main reason for this is that the EUL inversion (which is carried out in the complex-domain) does not require the high numerical precision of the GS: A precision of 12 digits will suffice for the EUL, compared with the 80 
    ...
    \vspace{1em}
    \par  \pause The statistical results show that the difference between the EUL and GS results are small. So the  the EUL implementation is preferable, 
...
\begin{frame}{Barrier Options Con.}
    ...
    \par For the two-dimensional transform, the price obtained in both inverting $ \tilde{f}_{UIC}(\xi, \alpha)$ and inverting $ \tilde{f}_{\Psi_{UI}}(\xi, \alpha)$.
     \vspace{1em}
    \par \pause  The two-dimensional Laplace transform approach is more efficient, simpler, and numerically 
