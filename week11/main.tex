...
\begin{frame}{The standard implicit scheme}
    ...
    {\footnotesize \footnotesize
     \par For many financial applications, the explicit scheme turns out to be very
        ...
        \vspace{1em}
    \par We now evaluate the second derivative at time $(n+1)\Delta t$ instead of time $n\Delta t$,
\[
...
u_{i}^{0}=u_{0}(i\Delta x).
\]
\par Implementing an algorithm allowing to compute the approximation is less obvious here. 
...
\]
    ...
    \vspace{1em}
    \par Besides, the implicit scheme is clearly consistent with the heat equation and is stable because, like the explicit scheme, it satisfies the maximum principle
...
\]
\vspace{1em}
\par Observe that the choice $\theta = 0$ leads to the explicit scheme whereas the $\theta = 1$ yields the fully implicit scheme. For $\theta = 0.5$, the truncation error is of second order both in time and space. The condition for monotonicity and stability is here
...
\end{align*}
\par The consistency is straightforward, each finite difference approximates its derivative to leading order.
