...
\vspace{1em}
\par Proposition 3: The value function \( v \) is a viscosity supersolution to the HJB equation
\[
\beta v(x) - \sup_{u \in U} [Dv(x) \cdot f(x,u) + c(x, u)] = 0.
\]
\vspace{1em}
...
\]  
For any control policy \( u_t \in U \) and \( u_0 = u \), the dynamic programming principle from Proposition 2 yields  
\[
...
\begin{itemize}
    ...
    Then by the mean value theorem for integrals, the average converges.
     \item \[ \frac{\phi(X_t^y) - \phi(y)}{t} \to D\phi(y) \cdot f(y, u_0) \]
...
Now combine them and rearrange:
\[
...
\vspace{1em}
\par Therefore, adding the two inequalities leads to, \( 0 \leq t \leq t_y, \)  
\[
...
\]
The complication here is that the control policy \( u_s^{\varepsilon,t} \) depends on \( t \). Thus, we have to be careful when taking the limit \( t \to 0 \) in (14.8). For \( 0 \leq t \leq t_y \), we have by the Lipschitz condition of \( f \):
    }
...
\]
Similarly, the Lipschitz condition of \( c \) gives
\[
...
\begin{frame}{Uniqueness via the Comparison Principle}
    ...
    \beta v(x) - \sup_{u \in U} [Dv(x) \cdot f(x,u) + c(x, u)] = 0.
    \]
\par Theorem 2 is the most difficult part of the proof of Theorem 1. 
...
\vspace{1em}
\par We shall prove Theorem 2 by contradiction. Suppose there exists \( \delta > 0 \) 
...
\vspace{1em}
\par Consider a (doubling variable) function \(\Psi : \mathbb{R}^d \times \mathbb{R}^d \to \mathbb{R}\) defined as
\[
...
\begin{frame}{Uniqueness via the Comparison Principle}
    ...
    \[
\Psi(\bar{x}, \bar{y}; \varepsilon, \alpha, p) = \inf_{x, y} \Psi(x, y; \varepsilon, \alpha, p) \leq \Psi(z, z; \varepsilon, \alpha, p) \leq -\delta.
\]
...
\begin{frame}{Uniqueness via the Comparison Principle}
    ...
    {\footnotesize \footnotesize
      ...
      \end{align*}
      Note that for the first two terms of the above equation,
\[
\left| (f(\bar{x}, u_\varepsilon) - f(\bar{y}, u_\varepsilon)) \cdot \frac{\bar{x} - \bar{y}}{\varepsilon} \right| \leq \frac{1}{\varepsilon} \|f(\bar{x}, u_\varepsilon) - f(\bar{y}, u_\varepsilon)\| \cdot \|\bar{x} - \bar{y}\|
\]
\[
\leq \frac{L}{\varepsilon} \|\bar{x} - \bar{y}\|^2,
\]
\[
